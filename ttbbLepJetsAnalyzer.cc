...
private:
  ...
  float b_Lepton_pT;
  float b_Lepton_Eta;
  float b_Lepton_Iso03;
  float b_Lepton_Iso04;
  ...
  TTbarMC_ (iConfig.getUntrackedParameter<int>("TTbarSampleLabel", 0))
{
  ...
  tree->Branch("lepton_Iso03", &b_Lepton_Iso03, "lepton_Iso03/F" );
  tree->Branch("lepton_Iso04", &b_Lepton_Iso04, "lepton_Iso04/F" );

  tree->Branch("dphi", &b_dphi, "dphi/F");
  tree->Branch("mt", &b_mt, "mt/F");
  ...
  tree->Branch("bjet_Number" , &b_bJet_Number, "bjet_number/I" );
...
void ttbbLepJetsAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  int ch_tag  = 999;
  float isolation03=-999.;
  float isolation04=-999.;
  float mt = -999.;
  float dphi = -999.;
  ...
  if(selectedMuons.size()     == 1 &&
    ...
    isolation03= selectedMuons[0].relIso(0.3);
    isolation04= selectedMuons[0].relIso(0.4);
    mt = transverseMass( selectedMuons[0].p4(), MET->begin()->p4() );
    //met.SetPtEtaPhiE(MET.pt(), MET.eta(), MET.phi(), MET.E());
    dphi = lepton.DeltaPhi(MET->begin()->tlv());
    ch_tag = 0; //muon + jets
  ...
  if(selectedMuons.size()     == 0 &&
    ...
    isolation03= selectedElectrons[0].relIso();
    isolation04= selectedElectrons[0].relIso();
  ...
  if (ch_tag<2 && EvTrigger){ // Single lepton event
    ...
    b_Lepton_Iso03 = isolation03;
    b_Lepton_Iso04 = isolation04;
    b_mt = mt;
    b_dphi = dphi;
    ...
    for (unsigned int i = 0; i < JetIndex.size() ; i++) {
      ...
      if(goodJet && cleanJet){
       ...
       if( jet_btagDis_CSV > 0.890) nbjets++;
...
bool ttbbLepJetsAnalyzer::IsVetoElectron(const cat::Electron & i_electron_candidate)
{
  ...
  double ttbbLepJetsAnalyzer::transverseMass( const reco::Candidate::LorentzVector& lepton,
